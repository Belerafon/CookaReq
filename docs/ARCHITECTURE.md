...
### `app/`
- `main.py` — точка входа GUI: создаёт `wx.App`, инициализирует `ConfigManager`, `RequirementModel` и окно `MainFrame` с меню, контроллерами и агентом; принимает флаг `--list-panel-diag-level=0..10`, позволяющий по шагам отключать функции `ListPanel` при поиске регресса отображения.【F:app/main.py†L5-L64】
...
- Панели: `list_panel.py` (табличный список с фильтрами и контекстным меню; поддерживает диагностический уровень 0–10, который поэтапно отключает контекстное меню, рендер бейджей, фильтры, дополнительные колонки и сортировку, вплоть до голого `wx.ListCtrl` для расследования проблем отрисовки), `editor_panel.py` (форма редактирования требования с валидацией и вложениями), `document_tree.py` (иерархия документов), `navigation.py` (меню и хоткеи), `agent_chat_panel.py` (чат с `LocalAgent`); отдельное плавающее окно редактирования — `detached_editor.py` (переносит `EditorPanel` в отдельный `wx.Frame`, когда основной редактор скрыт).【F:app/ui/list_panel.py†L1-L742】【F:app/ui/editor_panel.py†L1-L81】【F:app/ui/document_tree.py†L1-L83】【F:app/ui/navigation.py†L1-L107】【F:app/ui/agent_chat_panel.py†L1-L77】【F:app/ui/detached_editor.py†L1-L86】
