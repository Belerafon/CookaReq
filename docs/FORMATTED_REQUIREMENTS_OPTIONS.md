...
- **DOCX formula renderer** — комбобокс выбора механизма рендера формул
  (MathML→OMML / KaTeX→MathML→OMML / PNG fallback) для экспорта в Word.
...
- **PNG формулы** (fallback): рендер формул в картинку и вставка в DOCX
...
2. **Word (DOCX) экспорт (карточки требований):**
   ...
   - Рендер формул выбирается пользователем через комбобокс (MathML→OMML /
     KaTeX→MathML→OMML / PNG fallback).
...
**Статус:** реализованы вставка изображения, добавление attachment и синхронизация attachments с текстом (включая опциональное удаление файлов при удалённых ссылках).
...
- [x] Итерация 1: введён формат attachments с `id`, копирование файлов в `assets/`, валидация уникальных ID и базовые тесты.
- [x] Итерация 2: добавлен режим Preview в редакторе с Markdown-рендером и подменой attachment-ссылок на локальные файлы; формулы рендерятся в Preview/HTML через MathML-конвертацию (без KaTeX/MathJax).
- [x] Итерация 3: HTML-экспорт рендерит Markdown (включая формулы), экспорт создаёт отдельную папку и копирует assets рядом, plain-text экспорт формирует ASCII-таблицы; DOCX поддерживает формулы в режиме MathML→OMML или как текст, а также PNG fallback.
- [x] Итерация 4: поиск и превью используют Markdown→plain text, превью statement сокращается до короткой версии.
- [x] Итерация 5: добавлена кнопка вставки изображения и синхронизация attachments с текстом (с опциональным удалением файлов).
- [x] Итерация 6: добавлены валидация Markdown (таблицы/HTML/схемы ссылок) и ограничения размеров для statement/attachments.
- [x] Итерация 7: обновлены README и подсказки редактора с синтаксисом Markdown и правилами формулировки.
