"""Helpers styling ``wx.SplitterWindow`` widgets consistently."""
...
class _SplitterHighlighter:
    ...
    def __init__(self, splitter: wx.SplitterWindow, accent: wx.Colour, thickness: int) -> None:
        ...
        self.accent = accent
        self._fallback_thickness = max(thickness, _MIN_THICKNESS)
    ...
    def _read_sash_size(self) -> int:
        getter = getattr(self.splitter, "GetSashSize", None)
        if callable(getter):
            try:
                size = int(getter())
            except Exception:
        ...
        if size <= 0:
            try:
                size = int(getattr(self.splitter, "SashSize", 0) or 0)
            except Exception:
                size = 0
        return size
    ...
    def _draw(self) -> None:
        ...
        if size.width <= 0 or size.height <= 0:
            return
        sash_size = self._read_sash_size()
        if sash_size <= 0:
            sash_size = self._fallback_thickness
