...
## Прогресс
- [+] Шаг 1: введены `AgentEvent` и `AgentEventLog`, `AgentRunPayload` сохраняет линейный журнал событий и строит `diagnostic.event_log` из него без дублирующих корневых полей; для хранения добавлен режим без повторной укладки event-log в diagnostic.
...
- [+] Шаг 5: унифицирован экспорт технического лога и переписки — при наличии event-log добавляются секции по шагам LLM и карточкам инструментов, повторяющиеся системные промпты режутся в полезной нагрузке событий, порядок сортировки совпадает с таймлайном.
- [+] Шаг 6: smoke‑тесты панели агента (`tests/gui/test_agent_chat_panel.py`) переходят на событийную модель: инструментальные карточки строятся из `AgentRunPayload.tool_results`, хронология событий не зависит от эвристик, отмена генерации сохраняет последний LLM‑шаг и event-log. Остаётся поддерживать миграцию старых историй при появлении новых требований.
...
## Статус работ
- Основные шаги плана выполнены: события агрегируются в `AgentEventLog`, сериализация и UI-таймлайн используют единую ленту без дубликатов, технический лог строится напрямую из событий и отражает ту же хронологию, что и переписка.
- Smoke-набор панели агента стабилизирован на новой модели событий; на будущее остаётся поддерживать миграцию истории при появлении новых требований и держать unit-тесты хронологии, чтобы не возвращались дубли и рассинхронизация.
