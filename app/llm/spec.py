...
SYSTEM_PROMPT = (
    dedent(
        ...
        If the prompt is purely conversational, or if the user already provided the exact text that needs to be translated inside the conversation, reply in natural language without calling a tool, matching the language used in the user request. When the user asks to translate requirements, statements, or any other workspace data that is only referenced by RID or context snapshots, you must first call `get_requirement` to fetch the latest content before answering.
