...
- Transcript rendering now uses a read-only multiline `wx.TextCtrl`. `AgentChatPanel._build_transcript_rows()` still flattens the timeline into timestamp/source/message triples, but the rows are concatenated into multi-line blocks separated by blank lines so long responses wrap naturally without horizontal clipping.
- The panel tracks block offsets alongside the rendered text, allowing context-menu and double-click actions to resolve back to the originating timeline entry. Regenerate requests therefore still target the correct `ChatEntry`, and keyboard users can invoke the same actions via the native context menu shortcut.
