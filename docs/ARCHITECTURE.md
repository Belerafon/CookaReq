...
- `splitter_utils.py` — общее оформление `wx.SplitterWindow`: подсветка и увеличение заметности ползунка разделения, чтобы пользователю было проще найти область перетаскивания. Используется в `main_frame.py` и `agent_chat_panel.py`.
