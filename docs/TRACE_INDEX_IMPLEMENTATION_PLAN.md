...
| Шаг 12. Export reports | Частично готово | Для `artifact-matrix` добавлены CLI/GUI JSON/CSV/HTML exports; CLI `--view report --format html` строит combined summary/diagnostics/matrix report. Остальные report views остаются позже. |
