...
- Transcript rendering is now driven by the event-oriented timeline from `app/ui/agent_chat_panel/view_model.py`. `TranscriptView` converts `ChatConversation` into `EntryTimeline` instances, preserving stable event identifiers, chronological ordering of MCP tool calls, and the raw LLM payload per entry. The diagnostic log reuses the same timeline so timestamps, prompts, responses, tool exchanges, and raw JSON appear in a single ordered feed. `TranscriptEntryPanel` renders each entry as a stacked sequence: the user bubble keeps the contextual collapsible, while an `agent-entry` container now emits one bubble per assistant message (intermediate tool-invocation steps and the final answer) before adding chronologically ordered collapsible sections for reasoning, the exact LLM request, every tool call (with nested request/raw panes), and the final raw payload. Tool diagnostics no longer live inside the bubble footer, which keeps the hierarchy “step → tool call → raw payload” explicit without coupling the bubble layout to diagnostic widgets.
- The timeline order relies exclusively on the immutable `sequence_index` assigned during collection, so cancelling an agent run or receiving late tool diagnostics cannot reshuffle existing bubbles; intermediate steps and final replies stay anchored to their original positions.
