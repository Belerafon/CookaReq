...
- Transcript rendering now relies on a standard `wx.dataview.DataViewListCtrl`. `AgentChatPanel._build_transcript_rows()` builds a simple sequence of rows (time, source, message text) from the timeline data produced by `build_conversation_timeline()`. User prompts, context snippets, reasoning segments, system diagnostics, and tool summaries are rendered as plain text rows, so the control behaves like a native list without custom diffing logic.
- Rows remain in conversation order and reuse the timeline metadata (timestamps, regenerated flags) to expose regenerate actions from the context menu and double-click handlers. The panel keeps per-row metadata so the controller can call back into `AgentRunController.regenerate_entry()` when the user requests a retry.
- Tool calls contribute compact textual summaries generated by `render_tool_summaries_plain()` with optional cost, duration, and argument details appended directly to the message text. Removing the bespoke `MessageBubble`/`ToolCallPanel` hierarchy eliminated custom paint/DC work and collapsible panes while keeping the underlying diagnostic information accessible in the transcript export.
