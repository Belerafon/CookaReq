...
- `agent_chat_panel.py` управляет беседами `ChatConversation` (`app/ui/chat_entry.py`), каждая из которых хранит пары `ChatEntry` c отметками времени вопроса/ответа; переписка теперь рисуется пузырями `TranscriptMessagePanel` (`app/ui/widgets/chat_message.py`) внутри `wx.lib.scrolledpanel.ScrolledPanel`: сообщения пользователя выравниваются вправо, агентские — влево, под заголовком отображается локальное время. Детали инструментов прячутся в `wx.CollapsiblePane` с кнопкой «Copy JSON». Панель хранит `_AgentRunHandle` с `CancellationEvent`, чтобы по кнопке «Стоп» мгновенно снять блокировку ввода и закрыть поток LLM на уровне клиента, а историю чата сохраняет в `agent_chats.json` подкаталога `.cookareq` внутри выбранного каталога требований, чтобы логи путешествовали вместе с документами.【F:app/ui/agent_chat_panel.py†L118-L287】【F:app/ui/agent_chat_panel.py†L360-L575】【F:app/ui/agent_chat_panel.py†L1001-L1033】【F:app/ui/chat_entry.py†L1-L168】【F:app/ui/widgets/chat_message.py†L1-L213】【F:app/util/cancellation.py†L1-L59】
