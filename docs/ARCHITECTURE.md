...
- Панели: `list_panel.py` (табличный список с фильтрами и контекстным меню; для отладки доступен уровень `list_panel_debug_level`, задаваемый из настроек или конфига — от `0` без ограничений до `27`, постепенно отключая расширенные функции. Верхние ступени разбирают отчётный режим по частям: после отключения очереди повторных попыток установки ширины колонок поочерёдно снимаются принудительные ширины, использование `wx.ListItem`, вызов `ClearAll`, пакетный `DeleteAllItems` и опора на автоматическую перерисовку; только после этого (уровень `26`) панель переключается на простой `wx.LC_LIST`, а на максимуме дополнительно снимается бокс-сайзер и любые обработчики. Перечень отключённых возможностей логируется как при создании, так и после инициализации окна), `editor_panel.py` (форма редактирования требования с валидацией и вложениями), `document_tree.py` (иерархия документов), `navigation.py` (меню и хоткеи), `agent_chat_panel.py` (чат с `LocalAgent`); отдельное плавающее окно редактирования — `detached_editor.py` (переносит `EditorPanel` в отдельный `wx.Frame`, когда основной редактор скрыт).【F:app/ui/list_panel.py†L1-L226】【F:app/ui/list_panel.py†L778-L852】【F:app/ui/editor_panel.py†L1-L81】【F:app/ui/document_tree.py†L1-L83】【F:app/ui/navigation.py†L1-L107】【F:app/ui/agent_chat_panel.py†L1-L77】【F:app/ui/detached_editor.py†L1-L86】
