...
- [>] Шаг 3: начата адаптация UI — построение таймлайна агента теперь читает ленту событий и ставит временные метки из `AgentEvent`, убраны попытки сдвигать время финального ответа относительно вызовов инструментов.
- [ ] Шаги 4–6: осталось переписать сериализацию `ChatEntry` и лог-экспорт под новый журнал событий, обновить UI-тесты и снепшоты.
