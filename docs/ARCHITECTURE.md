...
- `agent_chat_panel.py` управляет беседами `ChatConversation` (`app/ui/chat_entry.py`), каждая из которых содержит список запросов/ответов `ChatEntry`; переписка рисуется карточками `TranscriptMessagePanel` (`app/ui/widgets/chat_message.py`) внутри `wx.lib.scrolledpanel.ScrolledPanel`: для каждого вызова инструмента создаётся `wx.CollapsiblePane` с деталями `tool_results` и кнопкой копирования JSON.【F:app/ui/agent_chat_panel.py†L118-L263】【F:app/ui/agent_chat_panel.py†L360-L575】【F:app/ui/chat_entry.py†L1-L145】【F:app/ui/widgets/chat_message.py†L13-L209】
- `splitter_utils.py` — общее оформление `wx.SplitterWindow`: подсветка и увеличение толщины ползунка разделения, чтобы пользователю было проще найти область перетаскивания.
...
- `tools/gitlab_migrate/diagnostics.py` — набор утилит для миграционного скрипта GitLab. `DiagnosticsCollector` собирает сводку системных команд, но обращения к контейнеру (`gitlab-ctl`, `docker`) выполняются только по запросу или при явном разрешении.【F:tools/gitlab_migrate/diagnostics.py†L1-L138】
