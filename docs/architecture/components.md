...
- Transcript rendering is now driven by turn-oriented data from `app/ui/agent_chat_panel/view_model.py`. `TranscriptView` converts every `ChatEntry` into a `TranscriptEntry` that contains the prompt metadata and a single `AgentTurn` anchored to `response_at`. Each turn bundles streamed responses, the final message, reasoning fragments, the captured LLM request sequence, tool-call diagnostics, and the raw payload. `TranscriptEntryPanel` renders one user bubble followed by a single agent bubble stamped with the turn timestamp; streamed steps appear as additional bubbles, while reasoning, LLM request details, tool-call collapsibles, and the raw JSON live inside the same agent panel, so diagnostics stay attached to that turn without synthetic timestamps. Missing timestamps are surfaced as explicit “Timestamp unavailable” labels instead of falling back to conversation-level dates.
...
- Tool call panes show a compact overview: arguments and significant results render as a short bullet list, the localized error headline carries the MCP code when available, and the raw JSON collapsibles still expose the full payload. When the agent runtime fails to persist the LLM request we synthesise it from the recorded tool arguments so the summary, collapsible preview, and GUI tests can always surface what the model attempted to execute.
