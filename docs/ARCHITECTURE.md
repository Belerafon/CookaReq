...
- `agent_chat_panel.py` хранит историю в `ChatEntry` (`app/ui/chat_entry.py`) и рисует переписку карточками `TranscriptMessagePanel` (`app/ui/widgets/chat_message.py`) внутри `wx.lib.scrolledpanel.ScrolledPanel`: для каждого вызова инструмента создаётся `wx.CollapsiblePane` с деталями `tool_results` и кнопкой копирования JSON.【F:app/ui/agent_chat_panel.py†L116-L213】【F:app/ui/agent_chat_panel.py†L448-L505】【F:app/ui/chat_entry.py†L1-L65】【F:app/ui/widgets/chat_message.py†L13-L209】
