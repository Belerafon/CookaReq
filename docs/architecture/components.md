...
- Chat history now lives in a SQLite database (`agent_chats.sqlite`). The panel loads only conversation metadata during initialisation, keeps transcripts on disk, and appends a fresh draft conversation in memory so the operator can start typing without waiting for sidebar data loads.
...
- Transcript rendering is now driven by the timeline/segment layer in `app/ui/agent_chat_panel/view_model.py`. `ConversationTimelineCache` keeps an incremental snapshot of every `ChatConversation`, rebuilding only the `TranscriptEntry` objects that changed. `SegmentListView` consumes the cached `ConversationTimeline` and uses `build_entry_segments()` to update individual `TurnCard` widgets without freezing the scroller or recreating unaffected cards. Each turn renders through a `MessageSegmentPanel` for the prompt plus agent/tool diagnostics, so streaming updates keep the widget tree stable while reasoning steps, LLM requests, tool invocations, and raw payloads live on dedicated collapsible sections.
...
- The footer exposes an “Attach file…” action. The dialog now defaults to “All files” so the operator can pick any document; the panel reads the payload, verifies that it decodes as UTF-8 and passes a heuristic “plain text” check, and only then keeps the content in memory. Files larger than 1 MB are rejected up front with an error popup so that large binaries do not saturate memory. The accepted attachment is injected into the next agent prompt as an additional context message, and the buffer clears when the run starts. The summary label shows the shortened filename followed by a compact `size/tokens/usage` triplet (for example `notes.txt • 12KB/3kTok/0.8%`) while the tooltip expands those figures into full text with exact units so the operator can inspect details without widening the footer.
- The ℹ button in the footer opens `AgentProjectSettingsDialog`, which now manages both the project-specific system prompt and an optional documentation folder override. The override accepts absolute or relative paths (relative values resolve against the active requirements folder via `resolve_documents_root()`), letting teams dedicate a per-project knowledge base without touching the global MCP settings. `AgentChatPanel` keeps track of the global default and the project override separately, updating tooltips to show which source is active. Whenever a requirements folder is opened, `MainFrameAgentMixin` enumerates the effective documentation root with `UserDocumentsService` and injects the resulting ASCII tree into the `[User documentation]` block of the agent context so the LLM immediately sees the available reference material, read window, and token model.
...
- View model теперь отбрасывает снимки, которые не похожи на реальные вызовы MCP. Помимо отсутствия `tool_name`, `tool_call_id` и `tool_arguments`, фильтр дополнительно проверяет, что в полезной нагрузке есть осмысленные идентификаторы или аргументы (в том числе во вложенном блоке `tool`). Финальные ответы без инструментов больше не порождают пустые «Tool call — completed successfully» пузырьки: в истории остаются только настоящие MCP-интеракции, а общий `raw_result` продолжает попадать в секцию "Raw data".
