...
- The ℹ button in the footer opens `AgentProjectSettingsDialog` which now only manages the project-specific system prompt. Documentation access moved to the MCP settings tab: the “Documentation folder” field configures the user-visible directory exposed to tools, defaults to `share` relative to the active requirements folder, and is resolved centrally by `resolve_documents_root()`. `AgentChatPanel` keeps the resolved `documents_root` in sync with the MCP configuration and surfaces the path in tooltips so listeners (for example, documentation services) can react to updates via `set_documents_root_listener()`.
