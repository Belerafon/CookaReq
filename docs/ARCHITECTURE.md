...
- `splitter_utils.py` — общее оформление `wx.SplitterWindow`: подсветка и увеличение толщины ползунка разделения, чтобы пользователю было проще найти область перетаскивания; `MainFrame` и `AgentChatPanel` используют вспомогательный `SplitterEventBlocker`, чтобы временно глушить события перетаскивания во время программных изменений саша.
