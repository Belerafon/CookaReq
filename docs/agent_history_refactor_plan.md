...
- [>] Шаг 4: сериализация `ChatEntry` очищает диагностический `event_log`, нормализует `raw_result` через `AgentRunPayload` и выбрасывает дублирующие снапшоты инструментов; осталось подчистить хранение `tool_messages` и миграцию истории.
- [>] Шаг 5–6: обновить оставшиеся UI-тесты/снепшоты под событийную модель и убедиться, что экспорт истории не тянет устаревшие поля из diagnostic.
