...
> ⚠️ Для воспроизводимых запусков в контейнере сначала активируйте проектный venv:
> `source .venv/bin/activate && python -m pytest ...`.
> В ряде окружений `pytest` установлен только в `.venv`, поэтому `python3 -m pytest` может завершаться ошибкой `No module named pytest`.
...
| `quality-lint` | ~1 мин | При изменениях Python-кода и lint-конфигурации. | Статический анализ (`ruff`, `vulture`). |
| `quality-docstyle` | ~30 с | При изменениях docstring-ов и правил документации. | Проверка `pydocstyle` по целевому backend-скоупу. |
| `quality-i18n` | ~2 с | При изменениях UI-строк и файлов локализации. | Проверка полноты каталогов переводов. |
...
> Докстринговая проверка в `quality-docstyle` пока охватывает backend-модули (`app/core`, `app/util`, `app/mcp`, `app/cli`, а также
...
3. Перед релизом: `pytest --suite quality-lint -q`, `pytest --suite quality-docstyle -q`, `pytest --suite quality-i18n -q`, `pytest --suite real-llm -q` (с загруженными креденшелами).
