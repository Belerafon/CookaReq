...
class AgentChatHistory:
    ...
    def prune_empty_conversations(
        ...
        # При обычной очистке защищаем активный разговор, чтобы не выбросить
        # текущий черновик. При строгой верификации со стором удаляем пустые
        # разговоры даже если они активны: они не несут данных и мешают
        # автоматической уборке истории.
        protected_id = self._active_id
        if protected_id and not verify_with_store:
