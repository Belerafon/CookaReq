...
- Chat history now loads eagerly: the panel reads `agent_chats.json` during initialisation, merges the stored conversations into memory, and immediately appends a fresh draft conversation so the operator can start typing without waiting for a sidebar interaction.
