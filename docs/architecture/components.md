...
- The footer exposes an “Attach file…” action. The panel reads the selected UTF-8 text file into memory, injects it into the next agent prompt as an additional context message, and clears the buffer once the run starts. The summary label shows the shortened filename, file size in kilobytes, token usage, and the percentage of the context window that the attachment would consume.
