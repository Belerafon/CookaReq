...
- Ключ OpenRouter задаётся только в конфигурационных файлах.
  По умолчанию тесты используют мок LLM и не обращаются к внешнему API.
  Для интеграционных проверок создайте файл настроек с валидным ключом
  и вручную замените мок на реальный клиент.
