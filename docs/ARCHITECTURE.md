...
- `splitter_utils.py` — общее оформление `wx.SplitterWindow`: подсветка и увеличение заметности ползунка разделения, чтобы пользователю было проще найти область перетаскивания; также даёт контекстный менеджер `SplitterEventBlocker` для временного отключения обработчиков во время программных перемещений сашей. Применяется в `main_frame.py` и `agent_chat_panel.py`.
