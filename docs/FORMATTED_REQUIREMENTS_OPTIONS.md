...
- [~] Итерация 2: добавлен режим Preview в редакторе с Markdown-рендером и подменой attachment-ссылок на локальные файлы; рендер формул в Preview/HTML (KaTeX/MathJax) ещё не сделан.
- [~] Итерация 3: HTML-экспорт рендерит Markdown, экспорт создаёт отдельную папку и копирует assets рядом, plain-text экспорт упрощает Markdown; DOCX поддерживает формулы только в режиме MathML→OMML или как текст (нет SVG/PNG fallback).
...
- [ ] Итерация 6: миграции/валидаторы Markdown и ограничения размеров ещё не реализованы.
- [ ] Итерация 7: документация по синтаксису Markdown и локальные подсказки UI ещё не реализованы.
