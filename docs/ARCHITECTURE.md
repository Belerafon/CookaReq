...
- **GUI (`app/ui`)** — виджеты wxPython, которые работают поверх контроллеров и моделей, обмениваются данными через `RequirementModel` и `DocumentsController`. Исторически этот слой предоставлял доступ к агенту (`LocalAgent`), но во временной диагностической сборке `MainFrame` вместо панели агента создаёт дублирующий `ListPanel`, чтобы сравнить отображение требований в двух независимых экземплярах списка; дополнительно через меню `View → Debug Requirements List` можно открыть отдельное окно `DebugListFrame` с тем же компонентом, заполненным статическим набором строк для изоляции визуальных проблем, а под ним — «голый» `wx.ListCtrl` без кастомизаций, чтобы проверить, как нативный контрол ведёт себя на тех же данных.
...
### `app/ui/`
- `main_frame.py` — главное окно: собирает панели (`DocumentTree`, основной `ListPanel`, временный диагностический `ListPanel` вместо `AgentChatPanel`, `EditorPanel`), меню `Navigation`, настраивает `MCPController` и связку с `DocumentsController` для загрузки данных; отображает консоль логов с выбором уровня и усечением хвоста при росте истории. Управление шириной панелей полностью доверено стандартным `wx.SplitterWindow`: дерево документов и зеркальный список сворачиваются на лету, а в конфигурации сохраняются только базовые параметры окна и флаг видимости секций без подбора точных пиксельных значений. Метод `_load_directory` синхронизирует `MCPSettings.base_path` с выбранным каталогом и при необходимости перезапускает MCP-сервер, чтобы агент видел актуальные требования.【F:app/ui/main_frame.py†L1-L317】【F:app/config.py†L272-L311】【F:app/ui/main_frame.py†L1109-L1170】
...
- Панели: `list_panel.py` (табличный список с фильтрами и контекстным меню), `editor_panel.py` (форма редактирования требования с валидацией и вложениями), `document_tree.py` (иерархия документов), `navigation.py` (меню и хоткеи), временно отключённый `agent_chat_panel.py` (чат с `LocalAgent`, на время диагностики заменён на второй `ListPanel`), отдельное плавающее окно редактирования — `detached_editor.py` (переносит `EditorPanel` в отдельный `wx.Frame`, когда основной редактор скрыт). В диагностической конфигурации `ListPanel` теперь почти не отличается от «голого» `wx.ListCtrl`: `Title` фиксирована в нулевой колонке, остальные поля выводятся строго в порядке из конфигурации, строки заполняются чистым текстом без плашек/битмапов и ручной перекраски, а после массовых обновлений контрол ограничивается штатными `Refresh/Update` без собственных обработчиков `EVT_SIZE`. Это позволяет постепенно отключать кастомизации и сравнивать результат с нативным контролом на тех же данных.【F:app/ui/list_panel.py†L332-L414】【F:app/ui/list_panel.py†L581-L639】【F:app/ui/editor_panel.py†L1-L81】【F:app/ui/document_tree.py†L1-L83】【F:app/ui/navigation.py†L1-L107】【F:app/ui/agent_chat_panel.py†L1-L77】【F:app/ui/detached_editor.py†L1-L86】
...
- Разметка между панелями строится штатными `wx.SplitterWindow` прямо в `main_frame.py` и `agent_chat_panel.py`, дополнительные утилиты оформления не используются.
...
GUI традиционно использует LLM/MCP через чат-панель и настройки: `AgentChatPanel` создаёт `LocalAgent`, `SettingsDialog` умеет проверять соединение с помощью `LLMClient`/`MCPClient` и управлять `MCPController`. В диагностической сборке панель агента временно отключена в пользу зеркального `ListPanel`, однако конфигурация и сервисы MCP остаются на месте — после завершения отладки панель агента можно вернуть, не затрагивая остальные слои.
