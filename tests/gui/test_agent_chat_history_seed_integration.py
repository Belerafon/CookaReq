...
def test_demo_history_archive_activates_on_click(tmp_path, wx_app, gui_context, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="cookareq.ui.agent_chat_panel"):
        try:
            ...
            assert "segment_view.render.start" in phases
            assert "segment_view.render.summary" in phases
            summary = next(
                event for event in history_events if event["phase"] == "segment_view.render.summary"
            )
            assert summary.get("entry_count") >= 1
            assert summary.get("new_cards") >= 0
            ...
            assert "segment_view.render.start" in repeat_phases
            assert "segment_view.render.summary" in repeat_phases
            repeat_summary = next(
                event for event in repeat_events if event["phase"] == "segment_view.render.summary"
            )
            assert repeat_summary.get("entry_count") >= 1
        finally:
            frame.Destroy()
            _flush_events(wx_app)
