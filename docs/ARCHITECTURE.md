...
- Панели: `list_panel.py` (диагностическая обёртка над «голым» `wx.ListCtrl`, в которой остались только базовые колонки и текстовое заполнение без фильтров, контекстного меню и редактирования на месте), `editor_panel.py` (форма редактирования требования с валидацией и вложениями), `document_tree.py` (иерархия документов), `navigation.py` (меню и хоткеи), временно отключённый `agent_chat_panel.py` (чат с `LocalAgent`, на время диагностики заменён на второй `ListPanel`), отдельное плавающее окно редактирования — `detached_editor.py` (переносит `EditorPanel` в отдельный `wx.Frame`, когда основной редактор скрыт). Сейчас `ListPanel` строит только колонку `Title` и набор полей из конфигурации, переводит значения через `locale`, заполняет строки чистым текстом и полагается на стандартные `Refresh/Update` после любых изменений — никаких собственных обработчиков перерисовки или битмапов не осталось.【F:app/ui/list_panel.py†L1-L262】【F:app/ui/editor_panel.py†L1-L81】【F:app/ui/document_tree.py†L1-L83】【F:app/ui/navigation.py†L1-L107】【F:app/ui/agent_chat_panel.py†L1-L77】【F:app/ui/detached_editor.py†L1-L86】
