...
<<<<< codex/fix-display-issue-with-dash
- `agent_chat_panel.py` управляет беседами `ChatConversation` (`app/ui/chat_entry.py`), каждая из которых хранит пары `ChatEntry` c отметками времени вопроса/ответа; переписка теперь рисуется пузырями `TranscriptMessagePanel` (`app/ui/widgets/chat_message.py`) внутри `wx.lib.scrolledpanel.ScrolledPanel`: сообщения пользователя выравниваются вправо, агентские — влево, под заголовком отображается локальное время. Детали инструментов прячутся в `wx.CollapsiblePane` с кнопкой «Copy JSON». Ответы агента рендерятся как Markdown через `MarkdownContent`, который конвертирует текст в HTML с помощью `python-markdown` и показывает его в `wx.html.HtmlWindow`, поэтому таблицы, списки и кодовые блоки отображаются с форматированием и их можно выделить/скопировать. Перед выводом строки проходят через `normalize_for_display`, сглаживающий экзотические символы тире в UI и экспортируемых текстах. Панель хранит `_AgentRunHandle` с `CancellationEvent`, чтобы по кнопке «Стоп» мгновенно снять блокировку ввода и закрыть поток LLM на уровне клиента, а историю чата сохраняет в `agent_chats.json` подкаталога `.cookareq` внутри выбранного каталога требований, чтобы логи путешествовали вместе с документами.【F:app/ui/agent_chat_panel.py†L118-L287】【F:app/ui/agent_chat_panel.py†L360-L575】【F:app/ui/agent_chat_panel.py†L1001-L1033】【F:app/ui/chat_entry.py†L1-L168】【F:app/ui/widgets/chat_message.py†L1-L247】【F:app/ui/widgets/markdown_view.py†L1-L203】【F:app/ui/text.py†L1-L61】【F:app/util/cancellation.py†L1-L59】
======
- `agent_chat_panel.py` управляет беседами `ChatConversation` (`app/ui/chat_entry.py`), каждая из которых хранит пары `ChatEntry` c отметками времени вопроса/ответа; переписка теперь рисуется пузырями `TranscriptMessagePanel` (`app/ui/widgets/chat_message.py`) внутри `wx.lib.scrolledpanel.ScrolledPanel`: сообщения пользователя выравниваются вправо, агентские — влево, под заголовком отображается локальное время. Детали инструментов прячутся в `wx.CollapsiblePane` с кнопкой «Copy JSON». Перед отправкой запроса панель формирует системное сообщение с текущим документом и выбранными требованиями, чтобы LLM понимала формулировки вида «удали эти требования» или «впиши в это требование ...» и могла вызывать подходящие MCP-инструменты. Ответы агента рендерятся как Markdown через `MarkdownContent`, который конвертирует текст в HTML с помощью `python-markdown` и показывает его в `wx.html.HtmlWindow`, поэтому таблицы, списки и кодовые блоки отображаются с форматированием и их можно выделить/скопировать. Панель хранит `_AgentRunHandle` с `CancellationEvent`, чтобы по кнопке «Стоп» мгновенно снять блокировку ввода и закрыть поток LLM на уровне клиента, а историю чата сохраняет в `agent_chats.json` подкаталога `.cookareq` внутри выбранного каталога требований, чтобы логи путешествовали вместе с документами.【F:app/ui/agent_chat_panel.py†L118-L287】【F:app/ui/agent_chat_panel.py†L360-L575】【F:app/ui/agent_chat_panel.py†L1001-L1033】【F:app/ui/chat_entry.py†L1-L168】【F:app/ui/widgets/chat_message.py†L1-L247】【F:app/ui/widgets/markdown_view.py†L1-L203】【F:app/util/cancellation.py†L1-L59】
>>>>> main
