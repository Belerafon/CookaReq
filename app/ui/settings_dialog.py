...
LLM_HELP: dict[str, str] = {
    "message_format": _(
        ...
        " Chat Completions and OpenRouter interfaces. Keep this option when the"
        " provider does not document extended formats.\n\n"
        "• Harmony (GPT-OSS) — a structured protocol with system/developer/user/"
        " assistant/tool roles and analysis/commentary/final channels. Used by"
        " the gpt-oss family of models, it separates the chain of thought from"
        " the user-facing answer and clearly signals tool invocations. Requires"
        " a provider that accepts Harmony tokens (for example, the OpenAI"
        " Responses API or a compatible gateway). In streaming mode the model"
        " returns a sequence of events that must each be decoded via a Harmony"
        " renderer.\n\n"
        "• Qwen (ChatML + reasoning) — the format for Qwen-R/-MCP models with"
        " reasoning_content support. Dialogues are assembled in ChatML syntax"
        " (<|im_start|>role ...), and intermediate reasoning plus tool calls"
        " arrive directly inside the reasoning stream. CookaReq exposes the"
        " option in the UI, but full reasoning-tool support will ship after the"
        " Qwen renderer and parser are integrated.\n\n"
        "Practical guidance:\n"
        "— Always match the selected format with the provider's capabilities."
        " Picking an incompatible protocol results in errors or empty replies.\n"
        "— Harmony and Qwen need dedicated streaming parsers. If streaming is"
        " enabled, ensure that the LLM client can decode events for these"
        " formats.\n"
        "— When migrating to new models, lock the format in configuration to"
        " avoid falling back to OpenAI chat by accident.\n"
        "— When experimenting with Harmony, check that the system prompt"
        " enables Reasoning and lists valid channels; otherwise the model may"
        " degrade or ignore tool_calls.\n"
