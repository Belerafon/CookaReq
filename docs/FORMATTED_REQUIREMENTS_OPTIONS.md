...
**Статус:** пока реализована только вставка изображения с автоматическим добавлением attachment и Markdown-ссылки; синхронизация attachments с текстом отложена.
...
- [x] Итерация 2: добавлен режим Preview в редакторе с Markdown-рендером и подменой attachment-ссылок на локальные файлы.
- [x] Итерация 3: HTML-экспорт рендерит Markdown, экспорт создаёт отдельную папку и копирует assets рядом, plain-text экспорт упрощает Markdown.
- [x] Итерация 4: поиск и превью используют Markdown→plain text, превью statement сокращается до короткой версии.
- [x] Итерация 5: добавлена кнопка вставки изображения и синхронизация attachments с текстом.
