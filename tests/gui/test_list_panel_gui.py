...
def test_list_panel_uses_system_background_colour(wx_app):
    wx = pytest.importorskip("wx")
    import app.ui.list_panel as list_panel
    ...
    expected = wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW)
    if hasattr(panel.list, "GetBackgroundColour"):
        actual = panel.list.GetBackgroundColour()
        if hasattr(expected, "Get") and hasattr(actual, "Get"):
            assert actual.Get() == expected.Get()
        else:
            assert actual == expected
    if hasattr(panel, "GetBackgroundColour"):
        panel_colour = panel.GetBackgroundColour()
        if hasattr(expected, "Get") and hasattr(panel_colour, "Get"):
            assert panel_colour.Get() == expected.Get()
        else:
            assert panel_colour == expected
    frame.Destroy()


def test_list_panel_redraws_after_resize(wx_app):
    wx = pytest.importorskip("wx")
    import app.ui.list_panel as list_panel
    ...
    panel = list_panel.ListPanel(frame, model=RequirementModel())
    panel.set_columns(["title", "status"])
    panel.set_requirements([_req(1, "Req"), _req(2, "Another")])
    wx_app.Yield()

    initial_text = panel.list.GetItemText(0)
    size = panel.list.GetSize()
    panel.list.SetSize(wx.Size(size.width + 40, size.height + 20))
    wx_app.Yield()
    assert panel.list.GetItemText(0) == initial_text
    frame.Destroy()
