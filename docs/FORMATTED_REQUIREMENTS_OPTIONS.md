...
- **LaTeX → MathML → OMML**: через `pandoc` (если допустимо в окружении) либо
  через специализированные конвертеры.
...
- Риски: конвертация LaTeX → OMML может быть неполной; нужен выбор одной
...
- Или использовать `pandoc` для преобразования Markdown → DOCX, затем
  постобработка (например, привести структуру к карточкам).
- Плюсы: `python-docx` даёт контроль структуры карточек.
- Риски: pandoc добавляет внешнюю зависимость; python-docx сложнее для формул.
...
2. **Word (DOCX) экспорт (карточки требований):**
   ...
   - Обеспечить корректный рендер формул (например, через MathML/OMML
     конвертацию или KaTeX → MathML).
