...
def colour_alpha(colour: wx.Colour) -> int:
    """Return the alpha channel value for *colour* when available."""

    getter = getattr(colour, "Alpha", None)
    if callable(getter):
        try:
            return int(getter())
        except Exception:
...
def colour_to_hex(colour: wx.Colour) -> str:
    """Return ``colour`` formatted as ``#RRGGBB``."""
...
def resolve_control_icon_colour(window: wx.Window) -> wx.Colour:
    """Return a foreground colour suited for monochrome control icons."""
...
def _render_svg_with_bitmap_bundle(
    svg_markup: str, icon_size: wx.Size
) -> wx.Bitmap | None:
    """Render SVG markup via :mod:`wx.BitmapBundle` when supported."""
...
def _render_svg_with_wxsvg(svg_markup: str, icon_size: wx.Size) -> wx.Bitmap | None:
    """Render SVG markup using :mod:`wx.svg` as a compatibility fallback."""
...
def render_svg_bitmap(svg_markup: str, icon_size: wx.Size) -> wx.Bitmap | None:
    """Render *svg_markup* into a bitmap sized to ``icon_size``."""

    bitmap = _render_svg_with_bitmap_bundle(svg_markup, icon_size)
    if bitmap is None:
        bitmap = _render_svg_with_wxsvg(svg_markup, icon_size)
    return bitmap


def create_disabled_bitmap(bitmap: wx.Bitmap) -> wx.Bitmap | None:
    """Return a disabled-state bitmap derived from *bitmap* when possible."""

    if not bitmap or not bitmap.IsOk():
        return None
    try:
        image = bitmap.ConvertToImage()
        disabled = image.ConvertToDisabled()
        disabled_bitmap = wx.Bitmap(disabled)
    except Exception:
        return None
    if not disabled_bitmap or not disabled_bitmap.IsOk():
        return None
    return disabled_bitmap


def apply_button_bitmaps(
    button: wx.Button, bitmap: wx.Bitmap, disabled_bitmap: wx.Bitmap | None
) -> None:
    """Attach themed bitmaps to ``button`` accounting for disabled state."""
...
def clear_button_bitmaps(button: wx.Button) -> None:
    """Remove any bitmaps currently associated with ``button``."""
