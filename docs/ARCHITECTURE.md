...
### `app/ui/`
- `main_frame.py` — главное окно: собирает панели (`DocumentTree`, `ListPanel`, `EditorPanel`, `AgentChatPanel`), меню `Navigation`, настраивает `MCPController` и связку с `DocumentsController` для загрузки данных; отображает консоль логов с выбором уровня и усечением хвоста при росте истории. Управление шириной панелей полностью доверено стандартным `wx.SplitterWindow`: дерево документов и чат сворачиваются на лету, а в конфигурации сохраняются только базовые параметры окна и флаг видимости секций без подбора точных пиксельных значений. Метод `_load_directory` синхронизирует `MCPSettings.base_path` с выбранным каталогом и при необходимости перезапускает MCP-сервер, чтобы агент видел актуальные требования.【F:app/ui/main_frame.py†L1-L317】【F:app/config.py†L272-L311】【F:app/ui/main_frame.py†L1109-L1170】
...
- Панели: `list_panel.py` (табличный список с фильтрами и контекстным меню; для отладки доступен уровень `list_panel_debug_level`, задаваемый из настроек или конфига — от `0` без ограничений до `37`, постепенно отключая расширенные функции. Верхние ступени по одной снимают элементы отчётного режима: после отключения очереди повторных попыток установки ширины колонок последовательно отключаются принудительные ширины, `wx.ListItem`, вызовы `ClearAll` и пакетного `DeleteAllItems`, далее — выравнивание заголовков, «ленивая» перерисовка (подстрахована отложенным `Refresh`/`RefreshItems`, если стандартный апдейт не сработал), заполнение placeholder-текстом, принудительная запись текста в первой колонке и присоединение `ImageList`, работа с изображениями и привязка данных элементов. Следующие уровни отключают `RefreshItems`, затем прямой `Refresh()`, отдельно `Update()` и, наконец, дополнительный `SendSizeEvent`, что позволяет проверить гипотезы о некорректной перерисовке без переключения стиля. Лишь после этого (уровень `36`) панель переключается на простой `wx.LC_LIST`, а на максимуме (`37`) дополнительно снимается бокс-сайзер. Начиная с порога диагностики (`19`) каждое заполнение дополнительно пишет геометрию первых строк, ширины колонок и размеры списка — это помогает отследить, когда текст исчезает при сохранённом заголовке. Перечень отключённых возможностей логируется как при создании, так и после инициализации окна), `editor_panel.py` (форма редактирования требования с валидацией и вложениями), `document_tree.py` (иерархия документов), `navigation.py` (меню и хоткеи), `agent_chat_panel.py` (чат с `LocalAgent`); отдельное плавающее окно редактирования — `detached_editor.py` (переносит `EditorPanel` в отдельный `wx.Frame`, когда основной редактор скрыт).【F:app/ui/list_panel.py†L1-L226】【F:app/ui/list_panel.py†L600-L769】【F:app/ui/list_panel.py†L1084-L1269】【F:app/ui/editor_panel.py†L1-L81】【F:app/ui/document_tree.py†L1-L83】【F:app/ui/navigation.py†L1-L107】【F:app/ui/agent_chat_panel.py†L1-L77】【F:app/ui/detached_editor.py†L1-L86】
