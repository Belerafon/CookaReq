...
- [x] Итерация 3: HTML-экспорт рендерит Markdown (включая формулы), экспорт создаёт отдельную папку и копирует assets рядом, plain-text экспорт формирует ASCII-таблицы; DOCX поддерживает формулы в режиме MathML→OMML или как текст, а также PNG/SVG fallback.
